...
// Shared profile body used by both "My profile" (/) and the admin People profile
// (/people/[id]). Personal info on the left, to-dos (+ admin controls) on the right.
export async function ProfileView({ userId }: { userId: string }) {
  ...
  const user = await prisma.user.findUnique({
    ...
    include: {
      ...
      competencies: {
        ...
        include: { competency: true, assessedLevel: { include: { track: true } } },
      },
  ...
  if (!user) {
    return <div className="p-8"><div className="card p-8 text-center text-slate-500">User not found.</div></div>;
  }
  ...
  const showManage = perms.editIdentity || perms.setLevel;
  ...
  return (
    <div className="grid gap-6 p-8 lg:grid-cols-3">
      {/* Left column — personal information & framework data */}
      <div className="space-y-6 lg:col-span-2">
        ...
        <section className="card p-6">
          ...
          <dl className="grid grid-cols-1 gap-x-8 gap-y-3 text-sm sm:grid-cols-2">
            ...
            <Field label="Job family" value={user.jobFamily?.name ?? "—"} />
            <Field label="Track" value={user.track?.name ?? "—"} />
            <Field label="Level" value={user.level ? `${user.level.code} · ${user.level.label}` : "N/A"} />
      ...
      {/* Right column — to-dos & admin controls */}
      <div className="space-y-6">
        ...
        {showManage && (
          <section className="card p-6">
            <h2 className="mb-4 text-base font-semibold text-slate-900">Manage</h2>
                ...
                <div>
                  ...
                  <form action={setLevel} className="mt-1 space-y-2">
                    ...
                    <select name="levelId" defaultValue={user.levelId ?? ""} className="input" required>
                      ...
                      {levels.map((l) => (
                        <option key={l.id} value={l.id}>{l.code} · {l.label} ({l.track.name})</option>
                      ))}
                    </select>
...
function findLevelId(levels: { id: string; code: string }[], code: string): string {
